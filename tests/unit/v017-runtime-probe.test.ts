/**
 * v0.1.7 运行时集成探针（P0-1~P0-3 / P1-1~P1-3 / P0-5 / P0-5b 发布证据链）。
 *
 * 在 v0.1.6 全部探针（13 项）基础上，新增阶段 E 独立复验发现的 P0-5b 一项：
 * 14. revokeSaveControlInFlightDispatch → revoke 的 saveControl（epoch 落盘）in-flight
 *     期间 verify 恢复时，runTask 不得派发（内存 epoch 比较捕获：revoke 的 epoch+1 是
 *     同步内存修改，先于 saveControl await；check-latest-again 读 cache 仍旧 epoch 会
 *     返回 ok，必须由 runTask 二次确认的内存 epoch 比较兜底）
 *
 * 运行方式：作为 unit 测试执行（真实常量，不 mock capabilities——探针内临时启用/恢复）；
 * 通过后把结果写入 workspace 根 runtime-integration-evidence-v0.1.7.json
 * （v0.1.7 gate 校验该文件；stage-e 包包含）。
 */
...
describe('v0.1.7 运行时集成探针（v0.1.6 十三项 + P0-5b 一项全修复）', () => {
  it('全部探针通过并写入 runtime-integration-evidence-v0.1.7.json', async () => {
    ...
    // ---- 12. revoke 发生在 verify 挂起窗口内 → executor 不得被调用（P0-5）----
    {
      CAPABILITY_VERIFICATION.blockUser.verified = true;
      let whitelistCalls = 0;
      let armGate = false;
      ...
      const env = await makeRealEnv015(
        {},
        { execute: async () => { executed++; return { ok: true, status: 'ok' }; } },
        { backend },
      );
      const t = mkTask('p-revoke', 70, { authorization: makeAuth({ type: 'block' }) });
      ...
      const finalT = (raw['bb.queue'] as ActionTask[]).find((x) => x.id === 'p-revoke');
      results.revokeDuringVerifyExecutorCalls = executed;
      results.revokeDuringVerifyFinalStatus = finalT?.status;
      results.revokeDuringVerifyEpoch = env.queue.getStatus().authorizationEpoch;
      findings.revokeDuringVerifyDispatch =
        executed !== 0 || (finalT?.status !== 'cancelled' && finalT?.status !== 'skipped');
      expect(findings.revokeDuringVerifyDispatch).toBe(false);
      expect(executed).toBe(0);
      expect(['cancelled', 'skipped']).toContain(finalT?.status);
      CAPABILITY_VERIFICATION.blockUser.verified = false;
    }

    // ---- 13. cancel 发生在 verify 挂起窗口内 → executor 不得被调用（P0-5）----
    {
      CAPABILITY_VERIFICATION.blockUser.verified = true;
      let whitelistCalls = 0;
      let armGate = false;
      ...
      const t = mkTask('p-cancel', 71, { authorization: makeAuth({ type: 'block' }) });
      await base.set({ 'bb.queue': [t] });
      env.queue.adoptTasks([t]);
      armGate = true;
      whitelistCalls = 0;
      env.queue.kick();
      await waitFor(() => whitelistCalls >= 1);
      await env.coordinator.execute({ kind: 'cancelTasks', taskIds: ['p-cancel'] });
      gateWhitelist.resolve();
      await new Promise((r) => setTimeout(r, 200));
      const raw = await base.get(['bb.queue']);
      const finalT = (raw['bb.queue'] as ActionTask[]).find((x) => x.id === 'p-cancel');
      results.cancelDuringVerifyExecutorCalls = executed;
      results.cancelDuringVerifyFinalStatus = finalT?.status;
      findings.cancelDuringVerifyDispatch =
        executed !== 0 || (finalT?.status !== 'cancelled' && finalT?.status !== 'skipped');
      expect(findings.cancelDuringVerifyDispatch).toBe(false);
      expect(executed).toBe(0);
      expect(['cancelled', 'skipped']).toContain(finalT?.status);
      CAPABILITY_VERIFICATION.blockUser.verified = false;
    }

    // ---- 14. revoke 的 saveControl in-flight 期间 verify 恢复 → 不得派发（P0-5b）----
    {
      CAPABILITY_VERIFICATION.blockUser.verified = true;
      let whitelistCalls = 0;
      let armGate = false;
      ...
      const backend = {
        ...
        set: async (items: Record<string, unknown>) => {
          if (holdControlWrite && items['bb.queueControl'] !== undefined) {
            ...
            await gateControlWrite.promise; // revoke 的 saveControl 挂起（epoch 未落 cache）
          }
          await base.set(items);
        },
        remove: (keys: string[]) => base.remove(keys),
      ...
      const t = mkTask('p05b', 80, { authorization: makeAuth({ type: 'block' }) });
      await base.set({ 'bb.queue': [t] });
      env.queue.adoptTasks([t]);
      armGate = true;
      whitelistCalls = 0;
      ...
      const finalT = (raw['bb.queue'] as ActionTask[]).find((x) => x.id === 'p05b');
      results.revokeSaveControlInFlightExecutorCalls = executed;
      results.revokeSaveControlInFlightFinalStatus = finalT?.status;
      results.revokeSaveControlInFlightEpoch = (raw['bb.queueControl'] as { authorizationEpoch: number }).authorizationEpoch;
      findings.revokeSaveControlInFlightDispatch =
        executed !== 0 || (finalT?.status !== 'cancelled' && finalT?.status !== 'skipped');
      expect(findings.revokeSaveControlInFlightDispatch).toBe(false);
      expect(executed).toBe(0);
      expect(['cancelled', 'skipped']).toContain(finalT?.status);
      CAPABILITY_VERIFICATION.blockUser.verified = false;
    }
    ...
    writeFileSync(resolve(process.cwd(), 'runtime-integration-evidence-v0.1.7.json'), JSON.stringify(evidence, null, 2), 'utf8');
