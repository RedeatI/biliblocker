/**
 * v0.1.6 运行时集成探针（P0-1~P0-3 / P1-1~P1-3 / P0-5 发布证据链）。
 *
 * 在 v0.1.5 全部探针（11 项）基础上，新增阶段 E 独立复验发现的 P0-5 两项：
 * 12. revokeDuringVerifyDispatch → 撤权发生在 verifyTaskEligible 的 await 窗口内时，
 *     executor 不得被调用（stale verdict TOCTOU；epoch 检查移至末尾 + runTask 二次确认）
 * 13. cancelDuringVerifyDispatch → 取消发生在同一窗口内时，executor 不得被调用
 *
 * 运行方式：作为 unit 测试执行（真实常量，不 mock capabilities——探针内临时启用/恢复）；
 * 通过后把结果写入 workspace 根 runtime-integration-evidence-v0.1.6.json
 * （v0.1.6 gate 校验该文件；stage-e 包包含）。
 */
...
describe('v0.1.6 运行时集成探针（v0.1.5 十一项 + P0-5 两项全修复）', () => {
  it('全部探针通过并写入 runtime-integration-evidence-v0.1.6.json', async () => {
    ...
    const evidence = {
      schema: 'BILIBLOCKER_V0.1.6_RUNTIME_INTEGRATION_EVIDENCE_V1',
    ...
    writeFileSync(resolve(process.cwd(), 'runtime-integration-evidence-v0.1.6.json'), JSON.stringify(evidence, null, 2), 'utf8');
