/**
 * 最小复现 2：cancel（BB_CANCEL_TASKS）在 verify 挂起窗口内发生 → executor 是否仍执行
 * 统一根因验证：runTask 设置 in_flight 前无二次状态/epoch 检查
 */
...
async function main() {
  ...
  const backend: StorageBackend = {
    get: async (keys) => { if (keys.includes('bb.whitelist')) { whitelistCalls++; if (armGate && whitelistCalls === 1) await gateWhitelist.promise; } return base.get(keys); },
  ...
  let executed = 0;
  const queue = new ActionQueue({ repo, dedup, writer: coordinator.writer, executor: { execute: async (t: ActionTask): Promise<TaskResult> => { executed++; console.log(`  [executor] ${t.id} 已执行（官方请求发出）`); return { ok: true, status: 'ok' }; } } });
  coordinator.attachQueue(queue);
  await queue.start();

  const t1 = mkTask('c1', 61);
  ...
  console.log('[t] verify 挂起；用户点击「取消任务」');
  await coordinator.execute({ kind: 'cancelTasks', taskIds: ['c1'] });
  console.log('[t] cancel 完成；任务状态:', queue.pendingTasks().find((x) => x.id === 'c1')?.status);
  gateWhitelist.resolve();
  await new Promise((r) => setTimeout(r, 200));
  const finalRaw = await base.get(['bb.queue']);
  const finalT = (finalRaw['bb.queue'] as ActionTask[]).find((x) => x.id === 'c1');
  console.log('[t] 最终存储状态:', finalT?.status, '| executor 调用:', executed);
  console.log('[t] 结论:', executed > 0 ? 'P0 竞态复现：cancel 后 executor 仍执行' : '未复现');
