#!/usr/bin/env python3
"""
BiliBlocker v0.1.7 阶段 E 独立增强门禁（验收方独立编写，非开发者自检工具）。

用法：
    python review/stage-e-review/BiliBlocker-v0.1.7-independent-gate.py <workspace-root> --expected-version 0.1.7
...
SOURCE_EXCLUDED = {"node_modules", "out", "out-e2e", "dist", ".wxt", ".git", "test-results", "playwright-report", "coverage", ".workbuddy", "__pycache__", "review/__pycache__"}
...
def main() -> None:
    ...
    if clean_dir.is_dir():
        ...
        tolerated = {"runtime-integration-evidence-v0.1.7.json", "runtime-integration-evidence.json"}
    ...
    check(bi.get("version") == version, "build-info.version == 0.1.7")
    ...
    check(guard_ok, "runTask 在设置 in_flight 前二次检查 cancelled/revocationRequested（v0.1.7 必须）")
    # 14b（v0.1.7 / P0-5b）：二次确认必须含内存 epoch 比较——revoke 的 epoch+1 是同步
    #     内存修改（先于 saveControl await），存储写 in-flight 窗口内 cache 仍旧 epoch、
    #     任务仍 queued，只有内存 epoch 比较能兜底
    epoch_guard = False
    if inflight_idx >= 0:
        window = queue_src[max(0, inflight_idx - 600):inflight_idx]
        epoch_guard = "authorization.epoch !== this.control.authorizationEpoch" in window or "authorization?.epoch !== this.control.authorizationEpoch" in window
    check(epoch_guard, "runTask 二次确认含内存 epoch 比较（P0-5b：撤权发起后 executor 恒为 0）")
    ...
    dev_ev = root / "runtime-integration-evidence-v0.1.7.json"
