#!/usr/bin/env python3
"""
BiliBlocker v0.1.6 确定性发布证据门禁（Stage E 独立复验依据 v2）。

用法：
    python review/BiliBlocker-v0.1.6-release-gate.py <workspace-root> --expected-version 0.1.6

在 v0.1.5 门禁（版本/三 ZIP/SHA256SUMS/Source ZIP 逐文件一致/build-info/十步日志/
RELEASE-EVIDENCE/能力全 false + 11 项运行时探针 + 复验代码级检查）基础上，
新增 v0.1.6 代码级检查（对应阶段 E 独立复验 P0-5 及 v0.1.6 整改要求）：

1. runTask 设置 in_flight 前必须二次确认任务仍可执行（cancelled/revocationRequested 守卫）
   —— 修复 stale verdict TOCTOU（撤权/取消发生在 verify await 窗口内时 executor 不得被调用）。
2. verifyTaskEligible 的 epoch 检查位于所有 await 之后（check-latest-again，
   返回前用锁内最新 control repo.getQueueControl() 重比 authorization.epoch）。
3. 新增回归测试存在：tests/unit/v016-verify-revoke-race.test.ts（先红后绿）。
4. runtime-integration-evidence-v0.1.6.json 13 项 findings 全 false，
   含新增 revokeDuringVerifyDispatch / cancelDuringVerifyDispatch。
5. P1-4：ActionQueue.enqueue 标记 @internal（生产路径统一 planEnqueue）。
6. P1-5：runTask 捕获持久化异常（转 failed + lastError，无 orphaned promise）。
7. P1-6：revoke 先落盘 epoch（失败回滚）；任务落盘失败显式记录。
8. 冻结文档：docs/ACCEPTANCE-v0.1.4.md + docs/REMEDIATION-TRACE-v0.1.6.md。
...
SOURCE_FORBIDDEN_DIRS = ["node_modules", "out", "out-e2e", "dist", ".git", "test-results", "playwright-report", "coverage", ".wxt", ".workbuddy", "stage-e-review", "review/stage-e-review"]
SOURCE_EXCLUDED = {"node_modules", "out", "out-e2e", "dist", ".wxt", ".git", "test-results", "playwright-report", "coverage", ".workbuddy", "stage-e-review", "review/stage-e-review"}
...
def main() -> None:
    ...
    for doc in ["docs/ACCEPTANCE-v0.1.4.md", "docs/REMEDIATION-TRACE-v0.1.6.md"]:
    ...
    # ---- 14. 回归测试覆盖（v015 + v016 测试文件存在且含关键断言） ----
    ...
    # ---- 15. runtime-integration-evidence-v0.1.6.json（13 项缺陷全关闭） ----
    probe_file = root / "runtime-integration-evidence-v0.1.6.json"
    check(probe_file.is_file(), "提交 runtime-integration-evidence-v0.1.6.json")
    try:
        probe = json.loads(probe_file.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        fail("runtime-integration-evidence-v0.1.6.json 不是合法 JSON")
